<?php
/**
 * View: Таблиця реєстру членів ФСТУ.
 * Містить <thead> та порожній <tbody> (заповнюється через AJAX).
 * Також містить блок пагінації.
 *
 * Version:     1.0.0
 * Date_update: 2026-04-03
 *
 * @package FSTU\UserFstu\Views
 *
 * @var int  $current_year Поточний рік.
 * @var bool $is_logged_in Чи авторизований.
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

$prev_year = $current_year - 1;
?>

<div class="fstu-table-wrap" role="region" aria-label="Список членів ФСТУ" aria-live="polite">

	<!-- Індикатор завантаження -->
	<div class="fstu-loader fstu-hidden" id="fstu-loader" aria-hidden="true">
		<span class="fstu-loader__spinner"></span>
		<span class="fstu-loader__text">Завантаження...</span>
	</div>

	<table class="fstu-table" id="fstu-registry-table">
        <thead class="fstu-thead">
        <tr>
            <th class="fstu-th fstu-th--num" scope="col">№</th>
            <th class="fstu-th fstu-th--fstu" scope="col" title="Членство ФСТУ">ФСТУ</th>
            <th class="fstu-th fstu-th--card" scope="col">Чл. квиток</th>
            <th class="fstu-th fstu-th--wide-name" scope="col">
                <div class="fstu-th-with-search">
                    <span>ПІБ</span>
                    <input type="text" id="fstu-filter-search" class="fstu-input--in-header fstu-search-input" placeholder="🔍 Пошук...">
                </div>
            </th>
            <th class="fstu-th fstu-th--ofst" scope="col">ОФСТ</th>
            <th class="fstu-th fstu-th--club" scope="col">Клуб</th>

            <?php
            $can_view_sailing = \FSTU\Core\Capabilities::can_view_sailing_dues_columns();
            $colspan = $can_view_sailing ? 4 : 2;
            if ( $is_logged_in ) : ?>
                <th class="fstu-th fstu-th--dues" scope="col"><?php echo esc_html( $prev_year ); ?></th>
                <th class="fstu-th fstu-th--dues" scope="col"><?php echo esc_html( $current_year ); ?></th>
                <?php if ( $can_view_sailing ) : ?>
                    <th class="fstu-th fstu-th--dues" scope="col">віт-<?php echo esc_html( $prev_year ); ?></th>
                    <th class="fstu-th fstu-th--dues" scope="col">віт-<?php echo esc_html( $current_year ); ?></th>
                <?php endif; ?>
            <?php else : ?>
                <th class="fstu-th fstu-th--dues-locked" scope="col" colspan="<?php echo esc_attr( $colspan ); ?>" title="Доступно після входу">
                    <span class="fstu-lock-icon" aria-hidden="true">🔒</span>
                </th>
            <?php endif; ?>

				<th class="fstu-th fstu-th--actions" scope="col">
					<span class="fstu-sr-only">Дії</span>
					<button type="button"
					        class="fstu-th-settings-btn"
					        id="fstu-column-settings"
					        title="Налаштування стовпців"
					        aria-label="Налаштування стовпців">⚙</button>
				</th>
			</tr>
		</thead>

		<tbody class="fstu-tbody" id="fstu-registry-tbody">
			<!-- Заповнюється через AJAX -->
			<tr>
				<td colspan="11" class="fstu-no-results">
					<span class="fstu-loader__spinner"></span> Завантаження...
				</td>
			</tr>
		</tbody>
	</table>

</div><!-- .fstu-table-wrap -->

<!-- ── Пагінація ─────────────────────────────────────────────────────────── -->
<div class="fstu-pagination fstu-pagination--compact" id="fstu-pagination">
    <div class="fstu-pagination__left" style="white-space: nowrap;">
        <label class="fstu-pagination__per-page-label" for="fstu-filter-per-page" style="margin-right: 5px;">Показувати по:</label>
        <select id="fstu-filter-per-page" class="fstu-select fstu-select--compact fstu-filter-trigger" data-filter="per_page">
            <option value="10" selected>10</option>
            <option value="25">25</option>
            <option value="50">50</option>
            <option value="100">100</option>
        </select>
    </div>
    <div class="fstu-pagination__controls" id="fstu-pagination-controls">
        <button type="button" class="fstu-btn fstu-btn--page" id="fstu-page-first" disabled aria-label="Перша">«</button>
        <button type="button" class="fstu-btn fstu-btn--page" id="fstu-page-prev" disabled aria-label="Попередня">‹</button>
        <span id="fstu-page-numbers" class="fstu-pagination__numbers" style="display: flex; gap: 4px;"></span>
        <button type="button" class="fstu-btn fstu-btn--page" id="fstu-page-next" disabled aria-label="Наступна">›</button>
        <button type="button" class="fstu-btn fstu-btn--page" id="fstu-page-last" disabled aria-label="Остання">»</button>
    </div>
    <div class="fstu-pagination__info" style="white-space: nowrap;">
        <span id="fstu-stat-wrap" class="fstu-hidden" style="margin-right: 10px;">Сплатили: <strong id="fstu-stat-paid" style="color: var(--fstu-success);">0</strong> |</span>
        <span id="fstu-pagination-info-text"></span>
    </div>
</div>
<!-- .fstu-pagination -->
