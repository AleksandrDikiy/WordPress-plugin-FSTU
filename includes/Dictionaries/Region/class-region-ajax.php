<?php
/**
 * AJAX-обробники модуля "Довідник областей".
 *
 * Version:     1.0.2
...
class Region_Ajax {
	...
	public function init(): void {
		add_action( 'wp_ajax_fstu_region_get_list', [ $this, 'handle_get_list' ] );
		add_action( 'wp_ajax_fstu_region_get_single', [ $this, 'handle_get_single' ] );
		add_action( 'wp_ajax_fstu_region_create', [ $this, 'handle_create' ] );
		add_action( 'wp_ajax_fstu_region_update', [ $this, 'handle_update' ] );
		add_action( 'wp_ajax_fstu_region_delete', [ $this, 'handle_delete' ] );
		add_action( 'wp_ajax_fstu_region_get_protocol', [ $this, 'handle_get_protocol' ] );
	}
	...
	public function handle_get_list(): void {
		check_ajax_referer( Region_List::NONCE_ACTION, 'nonce' );
		...
		$count_sql = "SELECT COUNT(*) FROM vRegion WHERE {$where}";
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery,WordPress.DB.PreparedSQL.NotPrepared
		$total = (int) ( $params ? $wpdb->get_var( $wpdb->prepare( $count_sql, ...$params ) ) : $wpdb->get_var( $count_sql ) );

		$list_sql = "SELECT Region_ID, Region_Name, Region_NameEng, Region_Code, Region_Number, Region_Order
			FROM vRegion
			WHERE {$where}
			ORDER BY Region_Code ASC, Region_Name ASC
			LIMIT %d OFFSET %d";
	...
	public function handle_get_single(): void {
		...
		$item = $this->get_region_by_id( $region_id );
		if ( ! is_array( $item ) ) {
		...
		wp_send_json_success(
			[
				'item' => [
					...
					'region_name_eng'   => (string) ( $item['Region_NameEng'] ?? '' ),
					'region_code'       => (string) ( $item['Region_Code'] ?? '' ),
					'region_number'     => (string) ( $item['Region_Number'] ?? '' ),
					'region_order'      => (int) ( $item['Region_Order'] ?? 0 ),
					'region_date_create'=> (string) ( $item['Region_DateCreate'] ?? '' ),
				],
			]
		);
	}
	...
	public function handle_create(): void {
		...
		if ( null === $order ) {
			...
			$order = (int) $wpdb->get_var( 'SELECT COALESCE(MAX(Region_Order), 0) FROM S_Region' ) + 1;
		}
		...
		try {
			...
			$inserted = $wpdb->insert(
				'S_Region',
				[
					'Region_DateCreate' => current_time( 'mysql' ),
					'Region_Name'       => $data['region_name'],
					'Region_NameEng'    => $data['region_name_eng'],
					'Region_Code'       => $data['region_code'],
					'Region_Number'     => $data['region_number'],
					'Region_Order'      => $order,
				],
				[ '%s', '%s', '%s', '%s', '%s', '%d' ]
			);
			...
			if ( false === $inserted ) {
				throw new \RuntimeException( 'region_insert_failed' );
			}

			$this->log_action( self::LOG_TYPE_INSERT, sprintf( 'Додано нову область: %s', $data['region_name'] ), self::LOG_STATUS_SUCCESS );
			$this->commit_transaction();

			wp_send_json_success( [ 'message' => __( 'Область успішно додано.', 'fstu' ) ] );
		} catch ( \Throwable $exception ) {
			$this->rollback_transaction();
			$this->try_log_action(
				self::LOG_TYPE_INSERT,
				sprintf( 'Помилка додавання області: %s', $data['region_name'] ),
				'error'
			);
	...
	public function handle_update(): void {
		...
		$region_id      = absint( $_POST['region_id'] ?? 0 );
		$data           = $this->sanitize_form_data();
		$error_message  = $this->validate_form_data( $data, $region_id );
		...
		$order = null === $data['region_order']
			? (int) ( $item['Region_Order'] ?? 0 )
			: $data['region_order'];
		...
		try {
			...
			if ( false === $updated ) {
				throw new \RuntimeException( 'region_update_failed' );
			}

			$this->log_action( self::LOG_TYPE_UPDATE, sprintf( 'Оновлено область: %s', $data['region_name'] ), self::LOG_STATUS_SUCCESS );
			$this->commit_transaction();

			wp_send_json_success( [ 'message' => __( 'Область успішно оновлено.', 'fstu' ) ] );
		} catch ( \Throwable $exception ) {
			$this->rollback_transaction();
			$this->try_log_action(
				self::LOG_TYPE_UPDATE,
				sprintf( 'Помилка оновлення області: %s', (string) $item['Region_Name'] ),
				'error'
			);
	...
	public function handle_delete(): void {
		...
		$dependency_check = $this->check_delete_dependencies( $region_id );
		if ( ! $dependency_check['can_delete'] ) {
			$this->try_log_action( self::LOG_TYPE_DELETE, sprintf( 'Заблоковано видалення області: %s', (string) $item['Region_Name'] ), $dependency_check['status'] );
		...
		try {
			...
			$deleted = $wpdb->delete( 'S_Region', [ 'Region_ID' => $region_id ], [ '%d' ] );

			if ( false === $deleted || 1 !== (int) $deleted ) {
				throw new \RuntimeException( 'region_delete_failed' );
			}

			$this->log_action( self::LOG_TYPE_DELETE, sprintf( 'Видалено область: %s', (string) $item['Region_Name'] ), self::LOG_STATUS_SUCCESS );
			$this->commit_transaction();

			wp_send_json_success( [ 'message' => __( 'Область успішно видалено.', 'fstu' ) ] );
		} catch ( \Throwable $exception ) {
			$this->rollback_transaction();
			$this->try_log_action(
				self::LOG_TYPE_DELETE,
				sprintf( 'Помилка видалення області: %s', (string) $item['Region_Name'] ),
				'error'
			);
	...
	private function build_rows( array $items, int $offset, array $permissions ): string {
		...
		foreach ( $items as $item ) {
			...
			$region_id       = (int) ( $item['Region_ID'] ?? 0 );
			$region_name     = (string) ( $item['Region_Name'] ?? '' );
			$region_name_eng = (string) ( $item['Region_NameEng'] ?? '' );
			$region_code     = (string) ( $item['Region_Code'] ?? '' );

			$actions   = [];
			$actions[] = '<button type="button" class="fstu-region-dropdown__item fstu-region-view-btn" data-region-id="' . esc_attr( (string) $region_id ) . '">' . esc_html__( 'Перегляд', 'fstu' ) . '</button>';

			if ( ! empty( $permissions['canManage'] ) ) {
				$actions[] = '<button type="button" class="fstu-region-dropdown__item fstu-region-edit-btn" data-region-id="' . esc_attr( (string) $region_id ) . '">' . esc_html__( 'Редагування', 'fstu' ) . '</button>';
			}

			if ( ! empty( $permissions['canDelete'] ) ) {
				$actions[] = '<button type="button" class="fstu-region-dropdown__item fstu-region-dropdown__item--danger fstu-region-delete-btn" data-region-id="' . esc_attr( (string) $region_id ) . '">' . esc_html__( 'Видалення', 'fstu' ) . '</button>';
			}

			$html .= '<tr class="fstu-row">';
			$html .= '<td class="fstu-td fstu-td--num">' . esc_html( (string) $index ) . '</td>';
			$html .= '<td class="fstu-td fstu-td--name"><button type="button" class="fstu-region-link-button fstu-region-view-btn" data-region-id="' . esc_attr( (string) $region_id ) . '">' . esc_html( $region_name ) . '</button></td>';
			$html .= '<td class="fstu-td fstu-td--name-eng">' . ( '' !== $region_name_eng ? esc_html( $region_name_eng ) : '<span class="fstu-text-muted">—</span>' ) . '</td>';
			$html .= '<td class="fstu-td fstu-td--code">' . ( '' !== $region_code ? esc_html( $region_code ) : '<span class="fstu-text-muted">—</span>' ) . '</td>';
			$html .= '<td class="fstu-td fstu-td--actions">';
			$html .= '<div class="fstu-region-dropdown">';
			$html .= '<button type="button" class="fstu-region-dropdown__toggle" aria-expanded="false" title="' . esc_attr__( 'Меню дій', 'fstu' ) . '">▼</button>';
			$html .= '<div class="fstu-region-dropdown__menu">' . implode( '', $actions ) . '</div>';
			$html .= '</div>';
			$html .= '</td>';
			$html .= '</tr>';
		}
	...
	private function sanitize_form_data(): array {
		$order_raw = sanitize_text_field( wp_unslash( $_POST['region_order'] ?? '' ) );

		return [
			'region_name'         => $this->normalize_form_string( sanitize_text_field( wp_unslash( $_POST['region_name'] ?? '' ) ) ),
			'region_name_eng'     => $this->normalize_form_string( sanitize_text_field( wp_unslash( $_POST['region_name_eng'] ?? '' ) ) ),
			'region_code'         => $this->normalize_form_string( sanitize_text_field( wp_unslash( $_POST['region_code'] ?? '' ) ) ),
			'region_number'       => $this->normalize_form_string( sanitize_text_field( wp_unslash( $_POST['region_number'] ?? '' ) ) ),
			'region_order_raw'    => $order_raw,
			'region_order'        => '' === $order_raw ? null : absint( $order_raw ),
		];
	}
	 ...
	 * @param int                 $region_id Поточний ID для update.
	 */
	private function validate_form_data( array $data, int $region_id = 0 ): string {
		$name      = (string) ( $data['region_name'] ?? '' );
		$name_eng  = (string) ( $data['region_name_eng'] ?? '' );
		$code      = (string) ( $data['region_code'] ?? '' );
		$number    = (string) ( $data['region_number'] ?? '' );
		$order_raw = (string) ( $data['region_order_raw'] ?? '' );
		...
		if ( '' === $code ) {
			return __( 'Поле «Код» є обов’язковим.', 'fstu' );
		}

		if ( mb_strlen( $code ) > 50 ) {
			return __( 'Поле «Код» не може бути довшим за 50 символів.', 'fstu' );
		}
		...
		if ( '' !== $number && mb_strlen( $number ) > 50 ) {
			return __( 'Поле «Інформаційне поле» не може бути довшим за 50 символів.', 'fstu' );
		}
		...
		if ( $this->region_name_exists( $name, $region_id ) ) {
			return __( 'Запис з таким найменуванням уже існує.', 'fstu' );
		}

		if ( $this->region_code_exists( $code, $region_id ) ) {
			return __( 'Запис з таким кодом уже існує.', 'fstu' );
		}

		if ( '' !== $name_eng && $this->region_name_eng_exists( $name_eng, $region_id ) ) {
	...
	private function current_user_can_view(): bool {
		return Capabilities::current_user_can_view_region();
	}
	...
	private function current_user_can_manage(): bool {
		return Capabilities::current_user_can_manage_region();
	}
	...
	private function current_user_can_delete(): bool {
		return Capabilities::current_user_can_delete_region();
	}
	...
	private function current_user_can_protocol(): bool {
		return Capabilities::current_user_can_view_region_protocol();
	}
	 ...
	 */
	private function get_region_by_id( int $region_id ): ?array {
		global $wpdb;

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery
		$item = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT Region_ID, Region_DateCreate, Region_Name, Region_NameEng, Region_Code, Region_Number, Region_Order FROM S_Region WHERE Region_ID = %d LIMIT 1',
				$region_id
			),
			ARRAY_A
		);
	 ...
	 */
	private function region_name_exists( string $region_name, int $exclude_id = 0 ): bool {
		$rows   = $this->get_existing_region_rows();
		$needle = $this->normalize_compare_string( $region_name );

		foreach ( $rows as $row ) {
			$current_id = (int) ( $row['Region_ID'] ?? 0 );
			if ( $exclude_id > 0 && $exclude_id === $current_id ) {
				continue;
			}

			$current_name = $this->normalize_compare_string( (string) ( $row['Region_Name'] ?? '' ) );
	...
	/**
	 * Перевіряє, чи існує запис з таким кодом.
	 */
	private function region_code_exists( string $region_code, int $exclude_id = 0 ): bool {
		$rows   = $this->get_existing_region_rows();
		$needle = $this->normalize_compare_string( $region_code );
		...
		foreach ( $rows as $row ) {
			...
			$current_code = $this->normalize_compare_string( (string) ( $row['Region_Code'] ?? '' ) );
			if ( '' !== $current_code && $current_code === $needle ) {
				return true;
			}
		}
	 ...
	 */
	private function region_name_eng_exists( string $region_name_eng, int $exclude_id = 0 ): bool {
		$rows   = $this->get_existing_region_rows();
		$needle = $this->normalize_compare_string( $region_name_eng );
		...
		foreach ( $rows as $row ) {
			...
			$current_name_eng = $this->normalize_compare_string( (string) ( $row['Region_NameEng'] ?? '' ) );
	...
	/**
	 * Повертає наявні дані для duplicate-check.
	 *
	 * @return array<int,array<string,mixed>>
	 */
	private function get_existing_region_rows(): array {
		global $wpdb;

		// phpcs:ignore WordPress.DB.DirectDatabaseQuery
		$rows = $wpdb->get_results( 'SELECT Region_ID, Region_Name, Region_NameEng, Region_Code FROM S_Region', ARRAY_A );
	 ...
	 * @return array<string,mixed>
	 */
	private function check_delete_dependencies( int $region_id ): array {
		global $wpdb;
		...
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery
		$tables = $wpdb->get_col( $wpdb->prepare( $metadata_sql, $schema_name, 'Region_ID', 'S_Region', 'vRegion' ) );
		...
		$required_tables = [ 'S_City', 'S_Unit' ];
		$tables          = array_unique( array_merge( $required_tables, array_map( 'strval', $tables ) ) );

		/**
		 * Дає змогу розширити список таблиць / views, які треба перевірити перед delete.
		 *
		 * @param array<int,string> $tables Список об’єктів БД з колонкою Region_ID.
		 */
		$tables = apply_filters( 'fstu_region_dependency_tables', $tables );
		...
		foreach ( $tables as $table_name ) {
			...
			$sql = "SELECT COUNT(*) FROM `{$table_name}` WHERE `Region_ID` = %d";
			// phpcs:ignore WordPress.DB.DirectDatabaseQuery
			$count = (int) $wpdb->get_var( $wpdb->prepare( $sql, $region_id ) );
		...
		if ( ! empty( $dependencies ) ) {
			return [
				'can_delete' => false,
				'message'    => __( 'Область неможливо видалити, оскільки вона використовується в інших даних.', 'fstu' ),
	...
	private function log_action( string $type, string $text, string $status ): void {
		...
		if ( false === $inserted ) {
			throw new \RuntimeException( 'region_log_insert_failed' );
		}
	}
	...
	private function begin_transaction(): void {
		...
		if ( false === $result ) {
			throw new \RuntimeException( 'region_transaction_start_failed' );
		}
	}
	...
	private function commit_transaction(): void {
		...
		if ( false === $result ) {
			throw new \RuntimeException( 'region_transaction_commit_failed' );
		}
	}
