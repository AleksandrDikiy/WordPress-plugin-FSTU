...
class Member_Regional_Ajax {
	...
	public function handle_get_list(): void {
		...
		if ( '' !== $search ) {
			...
			$where   .= ' AND MemberRegional_Name LIKE %s';
			$params[] = $like;
		}

		$count_sql = "SELECT COUNT(*) FROM vMemberRegional WHERE {$where}";
		// phpcs:ignore WordPress.DB.DirectDatabaseQuery,WordPress.DB.PreparedSQL.NotPrepared
		$total = (int) ( $params ? $wpdb->get_var( $wpdb->prepare( $count_sql, ...$params ) ) : $wpdb->get_var( $count_sql ) );

		$list_sql = "SELECT MemberRegional_ID, MemberRegional_Name, MemberRegional_Order, MemberRegional_DateCreate, FIO
			FROM vMemberRegional
			WHERE {$where}
			ORDER BY MemberRegional_Order ASC, MemberRegional_Name ASC
			LIMIT %d OFFSET %d";
	...
	public function handle_create(): void {
		...
		if ( false === $inserted ) {
			$this->log_action( 'INSERT', __( 'Помилка додавання посади федерації.', 'fstu' ), $wpdb->last_error ?: 'DB error' );
			wp_send_json_error( [ 'message' => __( 'Помилка при збереженні запису.', 'fstu' ) ] );
		}

		$this->log_action( 'INSERT', sprintf( 'Додано посаду федерації: %s', $data['member_regional_name'] ), '✓' );
	...
	public function handle_update(): void {
		...
		if ( false === $updated ) {
			$this->log_action( 'UPDATE', sprintf( 'Помилка оновлення посади федерації: %s', (string) $item['MemberRegional_Name'] ), $wpdb->last_error ?: 'DB error' );
			wp_send_json_error( [ 'message' => __( 'Помилка при збереженні запису.', 'fstu' ) ] );
		}

		$this->log_action( 'UPDATE', sprintf( 'Оновлено посаду федерації: %s', $data['member_regional_name'] ), '✓' );
	...
	public function handle_delete(): void {
		...
		if ( false === $deleted ) {
			$this->log_action( 'DELETE', sprintf( 'Помилка видалення посади федерації: %s', (string) $item['MemberRegional_Name'] ), $wpdb->last_error ?: 'DB error' );
			wp_send_json_error( [ 'message' => __( 'Не вдалося видалити запис.', 'fstu' ) ] );
		}

		$this->log_action( 'DELETE', sprintf( 'Видалено посаду федерації: %s', (string) $item['MemberRegional_Name'] ), '✓' );
	...
	private function get_member_regional_by_id( int $member_regional_id ): ?array {
		...
		$item = $wpdb->get_row(
			$wpdb->prepare(
				"SELECT MemberRegional_ID, MemberRegional_Name, MemberRegional_Order, MemberRegional_DateCreate, FIO
				 FROM vMemberRegional
				 WHERE MemberRegional_ID = %d
				 LIMIT 1",
				$member_regional_id
			),
			ARRAY_A
