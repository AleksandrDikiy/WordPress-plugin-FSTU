...
class Member_Regional_List {
	...
	/**
	 * Рендерить HTML модуля довідника посад федерацій.
	 *
	...
	/**
	 * Повертає канонічний URL модуля Member Regional.
	 */
	public static function get_module_url( string $context = 'default' ): string {
		...
		/**
		 * Дозволяє перевизначити канонічний URL модуля Member Regional.
		 *
	...
	private function enqueue_script( array $permissions ): void {
		...
		wp_localize_script(
			...
			[
				...
				'table'       => [
					'colspan' => ! empty( $permissions['canAdminMeta'] ) ? 5 : 3,
				],
				'messages'    => [
	...
	/**
	 * Автоматично знаходить фронтенд-сторінку з shortcode [fstu_member_regional].
	 */
