<?php
/**
 * Контролер відображення модуля "Довідник керівних органів ФСТУ".
 * Реєструє шорткод [fstu_typeguidance], підключає скрипти/стилі,
...
class TypeGuidance_List {

	private const ASSET_HANDLE = 'fstu-typeguidance';

	public const SHORTCODE = 'fstu_typeguidance';

	private const ROUTE_OPTION = 'fstu_typeguidance_page_url';

	public const NONCE_ACTION = 'fstu_typeguidance_nonce';
	...
	/**
	 * Рендерить HTML модуля довідника керівних органів.
	 *
	...
	public function render_shortcode( array $atts = [] ): string {
		...
		if ( empty( $permissions['canView'] ) ) {
			return '<div class="fstu-alert">' . esc_html__( 'Немає прав для перегляду довідника керівних органів ФСТУ.', 'fstu' ) . '</div>';
		}
		...
		ob_start();
		include FSTU_PLUGIN_DIR . 'views/typeguidance/main-page.php';
		return ob_get_clean();
	}

	/**
	 * Повертає канонічний URL модуля TypeGuidance.
	 */
	public static function get_module_url( string $context = 'default' ): string {
		...
		/**
		 * Дозволяє перевизначити канонічний URL модуля TypeGuidance.
		 *
		 * @param string $url     Поточний URL.
		 * @param string $context Контекст використання URL.
		 */
		$filtered_url = apply_filters( 'fstu_typeguidance_module_url', $url, $context );
	...
	private function enqueue_assets(): void {
		...
		wp_enqueue_style(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'css/fstu-typeguidance.css',
			[],
			$ver
		);
		...
		wp_enqueue_script(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'js/fstu-typeguidance.js',
			[ 'jquery' ],
			$ver,
			true
		);
		...
		wp_localize_script(
			...
			[
				...
				'messages'    => [
					...
					'formAddTitle'  => __( 'Додавання керівного органу', 'fstu' ),
					'formEditTitle' => __( 'Редагування керівного органу', 'fstu' ),
					'formViewTitle' => __( 'Перегляд керівного органу', 'fstu' ),
				],
			]
		);
	}
	...
	/**
	 * Автоматично знаходить фронтенд-сторінку з shortcode [fstu_typeguidance].
	 */
