<?php
/**
 * Контролер відображення модуля "Довідник категорій походів".
 * Реєструє shortcode [fstu_hikingcategory], підключає assets
...
class HikingCategory_List {

	private const ASSET_HANDLE = 'fstu-hikingcategory';

	public const SHORTCODE = 'fstu_hikingcategory';

	private const ROUTE_OPTION = 'fstu_hikingcategory_page_url';

	public const NONCE_ACTION = 'fstu_hikingcategory_nonce';

	private const LEGACY_PATH = '/adm/HikingCategory/';
	...
	private function enqueue_assets(): void {
		...
		wp_enqueue_style(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'css/fstu-hikingcategory.css',
			[],
			FSTU_VERSION
		);
		...
		wp_enqueue_script(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'js/fstu-hikingcategory.js',
			[ 'jquery' ],
			FSTU_VERSION,
			true
		);
		...
		wp_localize_script(
			...
			[
				...
				'messages'    => [
					...
					'formAddTitle'   => __( 'Додавання категорії походів', 'fstu' ),
					'formEditTitle'  => __( 'Редагування категорії походів', 'fstu' ),
					'viewTitle'      => __( 'Перегляд категорії походів', 'fstu' ),
	...
	public static function get_module_url( string $context = 'default' ): string {
		...
		$filtered_url = apply_filters( 'fstu_hikingcategory_module_url', $url, $context );
	...
	private function render_module(): string {
		...
		ob_start();
		include FSTU_PLUGIN_DIR . 'views/hikingcategory/main-page.php';
		return ob_get_clean();
	}
