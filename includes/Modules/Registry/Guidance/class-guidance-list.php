...
/**
 * Контролер відображення модуля «Склад керівних органів ФСТУ».
 * Реєструє shortcode, підключає assets і локалізує фронтенд-дані.
 *
 * Version:     1.0.0
...
class Guidance_List {
	...
	private const ASSET_HANDLE = 'fstu-guidance';
	private const ROUTE_OPTION = 'fstu_guidance_page_url';

	public const SHORTCODE    = 'fstu_guidance';
	public const NONCE_ACTION = 'fstu_guidance_nonce';
	...
	public function render_shortcode( array $atts = [] ): string {
		...
		ob_start();
		include FSTU_PLUGIN_DIR . 'views/guidance/main-page.php';
	...
	public static function get_module_url( string $context = 'default' ): string {
		...
		$filtered_url = apply_filters( 'fstu_guidance_module_url', $url, $context );
	...
	private function enqueue_style(): void {
		wp_enqueue_style(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'css/fstu-guidance.css',
			[],
			$this->get_asset_version( FSTU_PLUGIN_DIR . 'css/fstu-guidance.css' )
		);
	}
	...
	private function enqueue_script( array $permissions ): void {
		wp_enqueue_script(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'js/fstu-guidance.js',
			[ 'jquery' ],
			$this->get_asset_version( FSTU_PLUGIN_DIR . 'js/fstu-guidance.js' ),
			true
		);
		...
		wp_localize_script(
			...
			[
				...
				'actions'     => [
					'getList'                  => 'fstu_guidance_get_list',
					'getSingle'                => 'fstu_guidance_get_single',
					'getFilters'               => 'fstu_guidance_get_filters',
					'getMemberGuidanceOptions' => 'fstu_guidance_get_member_guidance_options',
					'searchUsers'              => 'fstu_guidance_search_users',
					'create'                   => 'fstu_guidance_create',
					'update'                   => 'fstu_guidance_update',
					'delete'                   => 'fstu_guidance_delete',
					'getProtocol'              => 'fstu_guidance_get_protocol',
				],
				'messages'    => [
					...
					'positionsError'    => __( 'Не вдалося завантажити список посад.', 'fstu' ),
					'usersError'        => __( 'Не вдалося завантажити список користувачів.', 'fstu' ),
					...
					'cardDenied'        => __( 'Перегляд картки доступний лише авторизованим користувачам.', 'fstu' ),
					'formAddTitle'      => __( 'Додавання запису Guidance', 'fstu' ),
					'formEditTitle'     => __( 'Редагування запису Guidance', 'fstu' ),
					'viewTitle'         => __( 'Перегляд запису Guidance', 'fstu' ),
					'userHint'          => __( 'Введіть щонайменше 2 символи для пошуку.', 'fstu' ),
					'userEmpty'         => __( 'Користувачів не знайдено.', 'fstu' ),
					'userRequired'      => __( 'Оберіть користувача зі списку підказок.', 'fstu' ),
				],
			]
		);
	}
