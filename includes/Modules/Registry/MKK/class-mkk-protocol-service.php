...
/**
 * Сервіс протоколу модуля «Реєстр членів МКК ФСТУ».
 * Відповідає за strict/fallback логування та побудову рядків журналу.
 *
 * Version:     1.0.0
...
class MKK_Protocol_Service {
	...
	public function log_action_transactional( string $type, string $text, string $status = self::STATUS_SUCCESS ): void {
		...
		if ( false === $result ) {
			throw new \RuntimeException( 'mkk_log_insert_failed' );
		}
	}
