...
/**
 * Контролер відображення модуля «Реєстр членів МКК ФСТУ».
...
class MKK_List {
	...
	private const ASSET_HANDLE = 'fstu-mkk';
	private const ROUTE_OPTION = 'fstu_mkk_page_url';

	public const SHORTCODE    = 'fstu_mkk';
	public const NONCE_ACTION = 'fstu_mkk_nonce';
	...
	public function render_shortcode( array $atts = [] ): string {
		...
		ob_start();
		include FSTU_PLUGIN_DIR . 'views/mkk/main-page.php';
	...
	public static function get_module_url( string $context = 'default' ): string {
		...
		$filtered_url = apply_filters( 'fstu_mkk_module_url', $url, $context );
	...
	private function enqueue_style(): void {
		wp_enqueue_style(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'css/fstu-mkk.css',
			[],
			$this->get_asset_version( FSTU_PLUGIN_DIR . 'css/fstu-mkk.css' )
		);
	}
	...
	private function enqueue_script( array $permissions ): void {
		wp_enqueue_script(
			self::ASSET_HANDLE,
			FSTU_PLUGIN_URL . 'js/fstu-mkk.js',
			[ 'jquery' ],
			$this->get_asset_version( FSTU_PLUGIN_DIR . 'js/fstu-mkk.js' ),
			true
		);
		...
		wp_localize_script(
			...
			[
				...
				'actions'     => [
					'getList'     => 'fstu_mkk_get_list',
					'getSingle'   => 'fstu_mkk_get_single',
					'getFilters'  => 'fstu_mkk_get_filters',
					'searchUsers' => 'fstu_mkk_search_users',
					'create'      => 'fstu_mkk_create',
					'update'      => 'fstu_mkk_update',
					'delete'      => 'fstu_mkk_delete',
					'getProtocol' => 'fstu_mkk_get_protocol',
				],
				'messages'    => [
					...
					'formAddTitle'      => __( 'Додавання запису МКК', 'fstu' ),
					'formEditTitle'     => __( 'Редагування запису МКК', 'fstu' ),
					'viewTitle'         => __( 'Перегляд запису МКК', 'fstu' ),
					'userHint'          => __( 'Введіть щонайменше 2 символи для пошуку.', 'fstu' ),
					'userLockedEdit'    => __( 'У режимі редагування зміна члена ФСТУ у записі МКК першого релізу недоступна.', 'fstu' ),
					'userEmpty'         => __( 'Користувачів не знайдено.', 'fstu' ),
					'userRequired'      => __( 'Оберіть члена ФСТУ зі списку підказок.', 'fstu' ),
				],
			]
		);
	}
