...
/**
 * Сервіс протоколу модуля «Посвідчення членів ФСТУ».
 * Працює з журналом Logs модуля Application.
 *
 * Version:     1.0.0
 ...
 */
class Member_Card_Applications_Protocol_Service {

	public const LOG_NAME = 'Application';
	...
	public function log_action_transactional( string $type, string $text, string $status = self::STATUS_SUCCESS ): void {
		...
		if ( false === $result ) {
			throw new \RuntimeException( 'member_card_applications_log_insert_failed' );
		}
	}
