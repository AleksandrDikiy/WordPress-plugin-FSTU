...
/**
 * Сервіс протоколу модуля «Реєстратори».
 * Відповідає за запис і відображення журналу `Logs`.
 *
 * Version:     1.0.1
 ...
 */
class Recorders_Protocol_Service {

	public const LOG_NAME       = 'Recorders';
	...
	public function log_action_transactional( string $type, string $text, string $status = self::STATUS_SUCCESS ): void {
		...
		if ( false === $result ) {
			throw new \RuntimeException( 'recorders_log_insert_failed' );
		}
	}
	...
	public function log_view_action( string $text ): void {
		$this->try_log_action( 'V', $text, self::STATUS_SUCCESS );
	}
